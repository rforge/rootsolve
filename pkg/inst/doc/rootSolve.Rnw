...
@
The S3 plot method is used to generate the steady-state plot; R's function
\code{filled.contour} is used rather than \code{image} to do so.
<<label=steady2D, include=FALSE>>=
plot(ST3,main="2-D diffusion+production", image=FALSE, xlab="x", ylab="y")
@
\setkeys{Gin}{width=0.4\textwidth}
\begin{figure}
\begin{center}
