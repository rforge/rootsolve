...
\Keywords{roots of nonlinear equations, gradient, Jacobian, Hessian,
  steady-state, boundary value ODE, \proglang{R}}

\Plainkeywords{roots of nonlinear equations, gradient, Jacobian, Hessian,
  steady-state, boundary value ODE, R}
...
\Abstract{ \R package \rs \citep{rootSolve} includes
  \begin{itemize}
    ...
    \item Functions calculate the Jaobian matrix or - more general - the
      gradient of functions with respect to independent variables.
  \end{itemize}
}
...
\maketitle
  ...
  \[
    x_i ^*  \quad (i=1,n)\] for which \[f_j ({\mathbf{x}}^* ) =
        \mathbf{0} \quad(j=1,n)
  \]
...
\subsection{n equations in n unknowns}
  ...
  retrieve n roots of n nonlinear equations.
...
\subsection{n equations in n unknowns with known Jacobian}
  ...
  much more efficient to use one of the steady-state solvers (see next section)
  \code{stode} or \code{stodes}; the former in case the Jacobian is full or
  banded, the latter in case it has an arbitrary sparse structure.
  
  The steady-state solvers estimate the root of a system of ordinary
  differential equations (ODE) and require the (ODE) function to be called as
  \code{times,x,par}, where \code{x} are the unknowns to be estimated,
  \code{times} and \code{par} are the times and parameter values respectively.

  For functions that are not ordinary differential equations, the arguments
  \code{times} and \code{par} are considered as dummy arguments.
  
  As an example, a set of linear equations, comprising 500 unknowns are solved.
  Of course, one would do that using a nonlinear equation solver, but
...
@
  ...
  To use \code{stode} the nonlinear equation solver, it is noted that the
  ...
  is \code{function(times,x,pars)}
<<>>=
jfun <- function (t,x,p) A
@
  
  The function whose root is to be solved receives the current estimate of
  \code{x}, and returns the difference \code{A x-B}, as a \code{list}
<<>>=
fun <- function(t,x,p) list (A %*%x-B)
@

  Next \code{stode} is called with jactype = "fullusr" (it is a full Jacobian,
  and specified by the user):
<<>>=
print(system.time(X <- stode(y=1:500, func=fun, jactype="fullusr",
   jacfunc=jfun)))
@
  Finally, both are the same
<<>>=
sum((X1-X$y)^2)
@
