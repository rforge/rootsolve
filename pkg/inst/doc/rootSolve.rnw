...
ordinary differential equations (ODE). These functions are compatible with the ODE solvers in \ds, which solve initial
...
Although the graph (\ref{fig:one}) clearly demonstrates the existence of
...
locates the root iteratively, its values will depend on the
...
\ds, and requires the the ode's to be similarly specified.
...
\emph{steady.2D}, \emph{steady.band} find the steady-state by the
...
Thus the outcome will be biologically realistic.
...
a fraction of a second \footnote{on my old computer}.

The analytical solution of this equation is known; after plotting the numerical
approximation, it is added to the figure:
...
2 species (BOD, O2) (\code{nspec}); we force the result to be positive (\code{pos=TRUE}).
...
@
10 cells on the 2-D grid where there will be substance produced are randomly selected (\code{ii}).
\code{lrw=1000000} is the length of the work array needed by the solver. If this value
...
$\frac{\partial f(\mathbf{x})}{\partial x_i \partial x_j}$&\code{hessian}     &  Estimates the hessian matrix for a function f(x)   \\ \hline
\hline
\end{tabular}
\end{table*}
